KEGG.plot <-
function(KeggID, ratios = NULL,
  chipAnnotation = "hgu133plus2", expandGenes = FALSE, species = "Hs") {
 ...
 KEGGurl = paste("http://www.genome.jp/kegg-bin/download?entry=hsa", 
  KeggID, "&format=kgml", sep ="")
 KEGGpathway = parseKGML(KEGGurl)
 ...
 edgeTypes = sapply(keggGedgedata,function(x) {
  paste(unlist(sapply(getSubtype(x), function(y) getName(y))), collapse="_")
  })
 edgeLabel = ifelse(regexpr("phosphorylation",edgeTypes)>0,"P*",as.character(""))
 names(edgeLabel) = names(edgeTypes)
 
 # layout graph
 keggG <- layoutGraph(keggG, edgeAttrs = list(label = edgeLabel),
  layoutType = "dot") 
...
#   getDisplayName(x) }))[nodes(keggG)]
 ...
 nodesFontSize = 24 * median(nchar(nodesNames))/nchar(nodesNames) *
  sapply(gregexpr("\\\n",nodesNames), function(x) ifelse(any(x == -1), 1, length(x)+1) )
...
# ONE PIE SEGMENT PER GENE (PROBE SETS AVERAGED)
 ...
 nodeRenderInfo(keggG) <- list(label = nodesNames, shape = nodesShape,
  fill = nodesFill,
 ...
 if(!is.null(ratios)) { 
  ...
  nodeRenderInfo(keggG)$shape[nodesWithPie] = sapply(nodeRenderInfo(keggG)$shape[nodesWithPie],
   function(x) expressionNode)
 }

 edgeColor = factor(edgeTypes)
 edgeArrowhead = factor(edgeTypes)
 colorKey = c("green","palegreen","green","yellow",
  "purple", "grey", "cyan", "palegreen", "grey", 
  "red", "red", "grey", "grey", "orange",
  "orange", "red", "yellow", "orange")
 arrowheadKey = c("normal", "normal", "normal","dot",
  "diamond","normal", "odot", "normal", "normal", "tee",
  "tee", "normal", "none", "diamond",
  "diamond", "tee", "dot", "diamond")
#### to do: add dashed lines for indirect ####
 names(colorKey) = names(arrowheadKey) = c(
  "activation", "activation_indirect",
  "activation_phosphorylation", "binding/association",
  "compound", "dephosphorylation", "dissociation",
  "expression", "indirect", "inhibition",
  "inhibition_phosphorylation", "phosphorylation",
  "missing", "ubiquitination",
  "activation_ubiquination", "inhibition_dephosphorylation",       
  "phosphorylation_binding/association", "ubiquination_inhibition")
 ...
 edgeRenderInfo(keggG) <- list(arrowsize = "0.7", col = edgeColor, 
  fontsize = 10, textCol = edgeColor, arrowhead = edgeArrowhead)

 graphRenderInfo(keggG) = list(graph=list(rankdir="TB"), main = getTitle(KEGGpathway)) 
 renderGraph(keggG)
 
 if(exists('pathway.probes')) return(unlist(pathway.probes))
}
