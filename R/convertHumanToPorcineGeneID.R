...
# Ensembl BioMart based
 if (any(export == 0)) {
  ...
  tryCatch({
   ...
   SsENSG = getBM(attributes = c("ensembl_gene_id","TODO_homolog_ensembl_gene"), filters = "entrezgene_id",
    values = HsEG[export == 0], mart = Hs.ensembl)
   if(dim(SsENSG)[1] == 0) stop("No SsENSG.")
   SsEG = getBM(attributes =  c("ensembl_gene_id","entrezgene_id"), filters = "ensembl_gene_id",
